...
class ScoreTracker {
    ...
    computeHeight(physics) {
        ...
        let minMatterY = constants_1.GROUND_Y_PX; // "ground" in Matter.js coords
        for (const { y, h } of settled) {
            ...
            const topEdgeDisplayUnits = (constants_1.GROUND_Y_PX - topEdgeMatterY) / constants_1.UNIT;
            // Spec: exclude blocks below y = -5 display units.
            // A block below -5 display units has fallen off/under the floor region.
            if (topEdgeDisplayUnits < constants_1.SCORE_Y_FLOOR_UNITS) {
                excludedBlocks++;
                continue;
            }
            qualifyingBlocks++;
        ...
        const heightPx = constants_1.GROUND_Y_PX - minMatterY;
        const heightUnits = Math.max(0, heightPx / constants_1.UNIT);
